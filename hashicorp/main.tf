...
resource "aws_security_group" "mysql" {
  description = "security group for bankingbackend ${dEngine} created from terraform"
  vpc_id      = "${aws-vpc}"
...
resource "aws_instance" "web" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo docker run -it -p 8000:8000 -p 8080:8080 -e DATABASE_URL=jdbc:mysql://${aws_db_instance.default.endpoint}/bankingbackend theharbormaster/banking-on-spring-boot-3-5:latest"
    ]
  }
}
